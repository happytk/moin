...
d3.ptree.configuration = function() {
    return {
        ...
        data_keys: "deacon_ukyp1,deacon_ukyp2,deacon_ukyp3,deacon_ukyp4,deacon_ukyp5", //grouping대신 개별지정시에 사용
        data_grouping_cnt: 6, //datastorage에서 참고할 데이타의 group개수
        data_grouping_key: "inst_id", //datastorage에서 grouping_id
        data_get_url: "/__ptree/stat?", //datastorage url
        data_ref_url: "/__ptree/ref?", //datastorage url(reference)
        ...
        data_columns: [{
                key: 'Logrds',
                postfix: '',
                width: (document.body.offsetWidth - 20) / 16 * 4,
                height: 86,
                charttype: 'line'
            }, {
                key: 'ApCo',
                ...
                notifications: [{
                    limit: 30,
                    limit_chain_max: 2,
                    limit_time_range: ['0800', '2200'],
                    limit_days: [1, 2, 3, 4, 5]
                }]
            }, {
                key: 'CPU',
                postfix: '%',
                width: (document.body.offsetWidth - 20) / 16 * 8 / 3,
                height: 86,
                charttype: 'line',
                notifications: [{
                    limit: 95,
    ...
    d3.tsv(ref_url_t, function(sample_t) {
        ...
        var drawing_refchart = function(sample_y, sample_t) {
            ...
            if (sample_t && sample_y) {
                da.y.domain([
                    d3.min([d3.ptree.grouping_data_min(sample_t, da_key),
                        d3.ptree.grouping_data_min(sample_y, da_key)
                    ]) * 0.9,
                    d3.max([d3.ptree.grouping_data_max(sample_t, da_key),
                        d3.ptree.grouping_data_max(sample_y, da_key)
                    ]) * 1.1
                ]);
            } else if (sample_t) {
            ...
            da.pathgen = d3.svg.area().interpolate(cfg.interpolate_method)
                .x(function(d) {
                    return da.x(d.__time);
                })
                .y(function(d) {
                    return da.y(+d[da_key]) || 0;
                });
...
d3.ptree.rtchart = function(_cfg) {
    ...
    //default proc.
    if (cfg.data_key && +cfg.data_grouping_cnt > 0) {
        cfg.data_get_url = cfg.data_get_url + '&k=' + cfg.data_key + '&c=' + cfg.data_grouping_cnt;
    } else if (cfg.data_keys) {
        cfg.data_get_url = cfg.data_get_url + '&keys=' + cfg.data_keys
    } else {
        alert('Invalid data_get_url');
        return;
    }
        ...
        var keys = cfg.data_columns.map(function(d, i) {
            ...
            if (d.dailyref) {
                ...
                if (cfg.data_key && +cfg.data_grouping_cnt > 0) {
                    ...
                    d.dailyref_url = cfg.data_ref_url + '&keys=' + cfg.data_keys + '&dk=' + d.dailyref + '&sk=' + d.key;
                } else {
                    alert('Invalid data_get_url');
                    return;
                }
            ...
            function tick() {
                ...
                d3.tsv(cfg.data_get_url + '&last_dt=' + d3.ptree.std_dt_fmt(data_dt_last_max) + '&n=' + cfg.n, function(resp) {
                    ...
                    if (cfg.data_timing_key === null || cfg.data_timing_key === undefined) {
                        ...
                        now_max = d3.max(resp, function(d) {
                            return d.__time;
                        });
